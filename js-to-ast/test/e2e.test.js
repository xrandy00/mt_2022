...
test("", () => {
  ...
  expect(sut.findMatches(input, run.vulnerabilities)[0][0].title).toBe("Debug Vulnerability");
});
...
test("Unsafe defaults in `remark-html`", () => {
  ...
  expect(sut.findMatches(input, run.vulnerabilities)[0][0].title).toBe("Unsafe defaults in remark-html");
});